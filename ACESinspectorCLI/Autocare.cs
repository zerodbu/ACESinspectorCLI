...
namespace ACESinspectorCLI
{
    ...
    {// one of these holds the entire contents of the imported ACES file - along with the results of analysis. The methods for analysis are also here. the rationale for containerizing the data is for future 
        ...
        public void findIndividualAppErrors(analysisChunk chunk, VCdb vcdb, PCdb pcdb, Qdb qdb)
        {
            ...
            using (StreamWriter sw = new StreamWriter(cacheFilename))
            {
                foreach (App app in chunk.appsList)
                {
                    ...
                    if (errorString != "")
                    {
                        ...
                        string problemData = errorString + "\t" + app.id.ToString() + "\t" + app.basevehilceid.ToString() + "\t" + vcdb.niceMakeOfBasevid(app.basevehilceid) + "\t" + vcdb.niceModelOfBasevid(app.basevehilceid) + "\t" + vcdb.niceYearOfBasevid(app.basevehilceid) + "\t" + pcdb.niceParttype(app.parttypeid) + "\t" + pcdb.nicePosition(app.positionid) + "\t" + app.quantity.ToString() + "\t" + app.part + "\t" + app.niceFullFitmentString(vcdb, qdb);
                ...
                foreach (App app in chunk.appsList)
                {
                    ...
                    foreach (QdbQualifier myQdbQualifier in app.QdbQualifiers)
                    {
                        ...
                        {// QdbQualifier id not valid (nice name returned the numeric value of the id)
                            ...
                            sw.WriteLine("Invalid Qdb id (" + myQdbQualifier.qualifierId.ToString() + ")" + "\t" + app.id + "\t" + app.basevehilceid.ToString() + "\t" + vcdb.niceMakeOfBasevid(app.basevehilceid) + "\t" + vcdb.niceModelOfBasevid(app.basevehilceid) + "\t" + vcdb.niceYearOfBasevid(app.basevehilceid) + "\t" + pcdb.niceParttype(app.parttypeid) + "\t" + pcdb.nicePosition(app.positionid) + "\t" + app.quantity + "\t" + app.part + "\t" + app.niceAttributesString(vcdb, false) + "\t" + string.Join(";", app.notes));
                        }
                    }
                }
            }
                    ...
                    foreach (KeyValuePair<string, bool> searchTermEntry in noteBlacklist)
                    {
                        foreach (String note in app.notes)
                        {
                            ...
                            {//
                                ...
                                sw.WriteLine("Questionable note (" + note + ")" + "\t" + app.id + "\t" + app.basevehilceid.ToString() + "\t" + vcdb.niceMakeOfBasevid(app.basevehilceid) + "\t" + vcdb.niceModelOfBasevid(app.basevehilceid) + "\t" + vcdb.niceYearOfBasevid(app.basevehilceid) + "\t" + pcdb.niceParttype(app.parttypeid) + "\t" + pcdb.nicePosition(app.positionid) + "\t" + app.quantity + "\t" + app.part + "\t" + app.niceAttributesString(vcdb, false) + "\t" + string.Join(";", app.notes));
                            }
                        }
                    }
                }
            ...
            using (StreamWriter sw = new StreamWriter(cacheFilename))
            {
                ...
                {
                    if (app.action == "D") { continue; } // ignore "Delete" apps
                    if (!vcdb.vcdbBasevhicleDict.TryGetValue(app.basevehilceid, out basevidTemp))
                    {
                        chunk.basevehicleidsErrorsCount++;
                        sw.WriteLine(app.id + "\t" + app.basevehilceid.ToString() + "\t" + pcdb.niceParttype(app.parttypeid) + "\t" + pcdb.nicePosition(app.positionid) + "\t" + app.quantity + "\t" + app.part + "\t" + app.niceFullFitmentString(vcdb, qdb));
                    }
                }
            }
            ...
            using (StreamWriter sw = new StreamWriter(cacheFilename))
            {
                ...
                {
                    if (app.action == "D") { continue; } // ignore "Delete" apps
                    if (app.VCdbAttributes.Count > 0 && vcdb.vcdbBasevhicleDict.ContainsKey(app.basevehilceid))
                    {
                        ...
                        {// dont include invalid attributed apps in vcdb config analysis - these are handled in the "invalid attributes" analysis
                            ...
                            {   // this apps's combination of attribute values is not found in the specified VCdb.
                                ...
                                if (allowGraceForWildcardConfigs)
                                {
                                    ...
                                    {
                                        if (!vcdb.attributeHasWildcardInConfig(app.basevehilceid, attribute.name))
                                        {
        ...
        public int importXML(string _filePath, string _schemaString, bool respectValidateNoTag, bool importDeletes, Dictionary<string, string> noteTranslation, Dictionary<string, QdbQualifier> noteQdbTransformDictionary, VCdb vcdb)
        {
            ...
            using (FileStream xmlFileStream = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
        ...
        public string exportRelatedParts(string _filePath, PCdb pcdb, string leftType, string rightType, bool usePosition, bool useVCdbAttributes, bool useNotes, IProgress<int> progress)
        {
            ...
            // definition of "vehilce" is variable - basevehilceid+[position]+[vcdb-coded-qualifiers]+[notes]
            ...
            {// establish the reference (lefthand) list
                if (app.action == "D") { continue; } // ignore "Delete" apps
                vehicleKey = app.basevehilceid.ToString();
    ...
    public class VCdb
    {
        ...
        public List<int> findVehcileIDsForAppVCdbAttribute(App app, int attributeIndex)
        {
            ...
            if (!vcdbBasevhicleDict.ContainsKey(app.basevehilceid)) { return result; } // invalid basevids will not be present in the dict
